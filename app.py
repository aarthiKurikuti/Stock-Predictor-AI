...
def normalize_model_name(model_name: str) -> str:
    """Accept frontend/API model casing and return the canonical model name."""
...
def predict(ticker):
    ticker = ticker.upper()
    try:
        model = normalize_model_name(request.args.get('model', 'LSTM'))
...
def train(ticker):
    ...
    try:
        models = normalize_models(data.get('models', DEFAULT_MODELS))
    except ValueError as e:
        return jsonify({'error': str(e)}), 400
