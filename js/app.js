...
const state = {
  ...
  model: 'lstm',
  range: '3M',
  useMock: true, // Toggle this to true/false depending on environment
...
async function loadStock(ticker) {
  ...
  if (state.useMock) {
    ...
    try {
      // 1. Fetch historical data (using the fact that app.py serves 'results' JSONs)
      const res = await fetch(`/api/predict/${ticker}?model=${state.model.toUpperCase()}`);

      if (res.status === 202) {
        // Training triggered
        showToast(`Training model for ${ticker}... Please wait.`, 'info');
        // Trigger actual training via POST
        await fetch(`/api/train/${ticker}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ models: [state.model.toUpperCase()] })
        });
        ...
        for (let i = 0; i < 30; i++) { // poll for 30 seconds max
          await sleep(1000);
          const statusRes = await fetch(`/api/status/${ticker}`);
        ...
        if (Ready) {
          return loadStock(ticker); // reload now that it's ready
        } else {
          showToast(`Training taking longer than expected. Check back soon.`, 'warning');
          showLoading(false);
          return;
        }
      }
      ...
      if (res.ok) {
        ...
        // The API returns 'actual' and 'pred' arrays of the last 60 days
        // To keep frontend functionality, we synthesize enough historical data or adjust charts
        // For simplicity, let's generate mock OHLCV but override the recent predictions
        state.ohlcv = generateOHLCV(ticker, 400);
        state.preds = {
          ...
          mae: data.mae,
          mape: data.mape,
          predicted: new Array(state.ohlcv.length - 60).fill(null).concat(data.pred),
          confidence: Math.max(60, 95 - (data.rmse / data.next_day * 100))
        };
      } else {
