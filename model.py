...
def normalize_model_name(model_name: str) -> str:
    """Return the canonical model key used by BUILDERS and saved results."""
    normalized = MODEL_NAME_ALIASES.get(str(model_name).strip().lower())
    if not normalized:
...
def normalize_price_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize yfinance OHLCV columns across simple and MultiIndex outputs."""
    if isinstance(df.columns, pd.MultiIndex):
        price_columns = {'open', 'high', 'low', 'close', 'adj close', 'volume'}
        price_level = 0
        for level in range(df.columns.nlevels):
            values = {str(value).strip().lower() for value in df.columns.get_level_values(level)}
            if values & price_columns:
                price_level = level
                break
        df = df.copy()
        df.columns = [str(column[price_level]).strip().lower() for column in df.columns]
    else:
        df = df.copy()
    ...
    if missing:
        ...
        raise ValueError(f"Downloaded data is missing required columns: {missing_cols}")
    return df
...
def fetch_data(ticker: str, period: str = "2y") -> pd.DataFrame:
    ...
    return normalize_price_columns(df).sort_index()
...
def preprocess(df: pd.DataFrame):
    ...
    if len(df) <= LOOKBACK:
        raise ValueError(
            f"Not enough valid rows after indicator calculation. Need more than {LOOKBACK}, got {len(df)}."
        )
    ...
    if len(df) <= LOOKBACK:
        raise ValueError(
            f"Not enough rows after outlier filtering. Need more than {LOOKBACK}, got {len(df)}."
        )
    ...
    if len(X) < 2:
        raise ValueError("Not enough training sequences after preprocessing.")
...
def run_pipeline(ticker: str, models: list = None, save_dir: str = 'results'):
    ...
    """
    ticker = ticker.upper()
    models = normalize_model_list(models or DEFAULT_MODELS)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models', nargs='+', default=DEFAULT_MODELS,
                        help='Models to train: LSTM, GRU, Transformer')
